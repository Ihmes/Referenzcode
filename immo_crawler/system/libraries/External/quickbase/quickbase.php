...
'.$page_id.'
	...
	public function import_from_csv ($records_csv, $clist, $skip_first = 0) {
		if($this->xml) {
			...
			//SB Added
			//foreach($records_csv as $oCsv){
			//  $oXMLField = $xml_packet->addChild('field',$oCsv['value']);
			//    $oXMLField->addAttribute('fid', $oCsv['fid']);
			//    echo $oXMLField."<br>";
			//}
	...
	/* API_CreateDatabase: http://www.quickbase.com/api-guide/index.html#create_database.html */
	public function create_table($dbid, $apptoken = 0, $tname, $pnoun) {

		if($this->xml) {
			$this->set_database_table($dbid);
			$xml_packet = new SimpleXMLElement('<qdbapi></qdbapi>');
			$xml_packet->addChild('apptoken',$apptoken);
			$xml_packet->addChild('tname',$tname);
			$xml_packet->addChild('pnoun',$pnoun);
			$xml_packet->addChild('ticket',$this->ticket);
			$xml_packet = $xml_packet->asXML();
			$response = $this->transmit($xml_packet, 'API_CreateTable');
		}
		else {
			$url_string = $this->qb_ssl . $dbid. "?act=API_CreateTable&ticket=". $this->ticket
				."&apptoken=".$apptoken
				."&tname=".$tname
				."&pnoun=".$pnoun;
